...
const details = () => {
  return {
    ...
    Operation: "Transcode",
    Description:
      "In a single pass ensures all files are in MKV containers and where possible encoded in h265 (settings dependant on file bitrate), converts all audio to AAC, and removes subtitles that are not in the configured language. This plugin is opinionated based on how I like my library to be configured and based on the work done by Migz with his plugins (Thanks!).",
    Version: "1.0",
    Tags: "pre-processing,ffmpeg,vaapi,h265,aac,nvenc",
    Inputs: [
      {
        name: "nvenc",
        type: 'string',
...
function buildAudioConfiguration(_inputs, file, logger) {
  ...
  var hasNonAacAudio = false;
  loopOverStreamsOfType(file, "audio", function (stream, id) {
    if (stream.codec_name !== "aac") {
      configuration.AddOutputSetting(`-c:a:${id} aac`);
      hasNonAacAudio = true;
    }
  });

  if (hasNonAacAudio) {
    logger.AddError("Will convert audio to AAC");
  }

  if (!hasNonAacAudio) {
    logger.AddSuccess("Audio already exists in AAC");
  }
